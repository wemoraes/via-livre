...
import { MapPin, Car, CheckCircle2, ChevronRight, XCircle } from "lucide-react";
import { formatLessonDateTime, formatCountdown } from "@/lib/datetime";
import type { NextLessonData } from "../_data/dashboard";

export default function NextLessonCard({ lesson }: { lesson: NextLessonData }) {
  ...
  return (
    <section className="glass-card rounded-2xl p-6 mb-6">
      ...
      <ul className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-5">
        ...
        <li className="flex items-start gap-2 text-sm" style={{ color: "var(--vl-text-2)" }}>
          <span className="text-xs font-bold" style={{ color: "var(--vl-text-3)", marginTop: 2 }}>R$</span>
          <span style={{ fontFamily: "var(--font-jetbrains-mono), monospace", fontWeight: 600 }}>
            {lesson.priceAmount.toFixed(2).replace(".", ",")}
          </span>
        </li>
      </ul>
